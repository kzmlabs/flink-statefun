...
An empty key is deliberately not a defect. It is a legal Kafka key and routes as a normal address to the function instance with id `""`, exactly as an explicit shared id would. Producers that do not intend all such records to converge on one instance must set a real key; the ingress does not second-guess an address it was given. The policy above therefore classifies a null key and a null value (tombstone) as invalid. A deserialization-failure defect class is reserved for a future revision: the routable ingress today treats payload bytes opaquely, so a payload decode failure is not observable at this layer.
...
- The default behavior changes from crash-the-job to skip-with-WARN-log; this is a breaking behavior change that must be called out in release notes. The skip-on-null contract applies to every KafkaIngressDeserializer wrapped by the runtime, including custom embedded-SDK ones: a null return, previously a broken-contract crash, now counts and drops the record.
- Operators who currently alert on job restarts as their signal for bad data need to migrate that alert to the `numInvalidRecordsSkipped` metric instead. Metric naming follows Flink conventions (camelCase `numXxx`, cf. `numLateRecordsDropped`); the counters register in the KafkaSource-provided `deserializer` subgroup. The originally proposed reuse of the FLIP-33 `numRecordsInErrors` counter was dropped in implementation (2026-08-08): that counter lives on the operator I/O group, unreachable from the deserializer context, and a same-named counter in a different scope would mislead dashboards. A per-topic, per-defect breakdown `topic.<name>.defect.<NULL_KEY|NULL_VALUE>.numInvalidRecordsSkipped` (the Kafka connector's group convention) makes one bad producer attributable and alertable in isolation (added 2026-08-08).
